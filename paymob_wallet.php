<?php
// paymob_wallet.php (Termux-ready)
date_default_timezone_set('Africa/Cairo');
$logFile = __DIR__ . '/logs/paymob_wallet.log';
if (!file_exists(dirname($logFile))) mkdir(dirname($logFile), 0755, true);
function write_log($m){ global $logFile; file_put_contents($logFile, "[".date('Y-m-d H:i:s')."] $m\n", FILE_APPEND); }
...
$integrationIdWallet = 5305757;
$iframeId = 962734;

// ===== خريطة الأسعار (المبالغ بالقرش) =====
...
write_log("Start wallet payment. plan=$plan amount=$amount_cents");
...
// 1) Auth
$auth = post_json("https://accept.paymob.com/api/auth/tokens", ["api_key"=>$apiKey]);
write_log("Auth code={$auth['code']} err={$auth['err']} resp={$auth['resp']}");
...
$order = post_json("https://accept.paymob.com/api/ecommerce/orders", $orderPayload);
write_log("Order code={$order['code']} resp={$order['resp']}");
$orderData = json_decode($order['resp'], true);
if (!isset($orderData['id'])) { echo "Order creation failed. check logs."; exit; }
$order_id = $orderData['id'];

// 3) Payment Key - billing_data كاملة (لتفادي أخطاء)
$billingData = [
    "apartment" => "NA",
...
$pk = post_json("https://accept.paymob.com/api/acceptance/payment_keys", $paymentKeyPayload);
write_log("PaymentKey code={$pk['code']} resp={$pk['resp']}");
...
write_log("Redirecting to $iframe_url");
